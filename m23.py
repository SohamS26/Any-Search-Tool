        ...
        for entry in os.scandir(directory):
            if entry.is_file(follow_symlinks=False):
                file_info = {
                    "name": entry.name,
                    "path": entry.path,
                    "size_mb": round(entry.stat(follow_symlinks=False).st_size / (1024 * 1024), 2),
                    "mod_time": datetime.datetime.fromtimestamp(entry.stat(follow_symlinks=False).st_mtime).strftime('%Y-%m-%d %H:%M:%S')
                }
                file_index[entry.path] = file_info
            elif entry.is_dir(follow_symlinks=False):
                index_files(entry.path, max_depth, current_depth + 1)
...
def search_files(query):
    ...
    for path, file_info in file_index.items():
        if query_lower in os.path.normpath(path).lower():
...
def do_search(query):
    matching_files = search_files(query)
...
def initial_indexing():
    search_depth = 20  # Set a default search depth

    # Iterate over all available drives
...
def new_command():
    subprocess.Popen(["python", "m19.py"])
